...
{
	// code
	[self makeCheckImage]; // checkmage array is populated here
...
- (void)display
{
	...
	GLfloat square_position[12] = {
		...
		-2.0, 1.0, 0.0,	 // left top
		-2.0, -1.0, 0.0, // left bottom
		0.0, -1.0, 0.0}; // right bottom
	GLfloat square_position1[12] = {
	...
	// square 1
	mat4 translationMatrix = vmath::translate(0.0f, 0.0f, -5.5f);
	...
	glBindBuffer(GL_ARRAY_BUFFER, vbo_position_square);
	glBufferData(GL_ARRAY_BUFFER, sizeof(square_position1), square_position1, GL_DYNAMIC_DRAW);
	glBindBuffer(GL_ARRAY_BUFFER, 0);
